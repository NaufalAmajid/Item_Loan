...
$alat = $mysqli->query("SELECT * FROM barang") or die($mysqli->error);
?>
        <div id="container">
                    <?php
                    while ($row_alat = mysqli_fetch_array($alat))
                      ...
                      ?>
                        <div class="col-md-4" style="padding: 50px">
                            <img src="../images/<?= $row_alat['foto'] ?>" width="200px" height="200px">
                            <h4><?php  echo $row_alat['nama_brg']; ?></h4>
                            <p>Stok : <?php echo $row_alat['stok_brg']; ?></p>
                            <a href="cartfunction.php?act=add&amp;id_product=<?php echo $row_alat['id_brg']; ?>&amp;id_pembeli=<?php echo $_SESSION['id_anggota']; ?>&amp;ref=view_cart.php" class="btn btn-<?= $row_alat['stok_brg'] == 0 ? 'danger' : 'success' ?>" <?= $row_alat['stok_brg'] == 0 ? 'style="pointer-events: none; cursor: not-allowed;"' : '' ?>><font color="white"><?= $row_alat['stok_brg'] == 0 ? 'Stok Habis' : 'Pinjam Barang' ?></font></a>
                        </div>
    ...
    <div class="container">
        ...
        <footer>
            <div class="row">
                <div class="col-lg-12">
                    <p><center></center>Copyright &copy; Agus Setiawan</p>
                </div>
            </div>
        </footer>
